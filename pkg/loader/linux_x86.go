...
const (
	BootCsSelector = 2 * 8 // 16 (0x10)
	BootDsSelector = 3 * 8 // 24 (0x18)
	BootTrSelector = 4 * 8 // 32 (0x20)
)

func gdtEntry(flags, base, limit uint64) uint64 {
	return (((base) & 0xff000000) << (56 - 24)) |
		(((flags) & 0x0000f0ff) << 40) |
...
func build32bitGdt(page []byte) {
	binary.LittleEndian.PutUint64(page[2*8:], gdtEntry(0xc09a, 0, 0xfffff))
	binary.LittleEndian.PutUint64(page[3*8:], gdtEntry(0xc092, 0, 0xfffff))
	binary.LittleEndian.PutUint64(page[4*8:], gdtEntry(0x8089, 0, 0x0))
	binary.LittleEndian.PutUint64(page[5*8:], gdtEntry(0x0000, 0, 0x0))
}

func build64bitGdt(page []byte) {
	binary.LittleEndian.PutUint64(page[2*8:], gdtEntry(0xa09a, 0, 0xfffff))
	binary.LittleEndian.PutUint64(page[3*8:], gdtEntry(0xc092, 0, 0xfffff))
	binary.LittleEndian.PutUint64(page[4*8:], gdtEntry(0x808b, 0, 0xfffff))
	binary.LittleEndian.PutUint64(page[5*8:], gdtEntry(0x0000, 0, 0))
}

func buildPml4(page []byte, pgdAddr uint64, size int) {
	entry := (pgdAddr & 0x000ffffffffff000) | 0x3
	binary.LittleEndian.PutUint64(page[:8], entry)
}

func buildPgd(page []byte, pdeAddr uint64, size int) {
	entry := (pdeAddr & 0x000ffffffffff000) | 0x3
	binary.LittleEndian.PutUint64(page[:8], entry)
}

func buildPde(page []byte, size int) {
	max := size / 8
	for i := 0; i < max; i++ {
		entry := (((uint64(i)) << 21) & 0x000fffffffe00000) | 0x83
		binary.LittleEndian.PutUint64(page[i*8:], entry)
	}
}
...
func SetupLinux(
	...
	if is_64bit {
		...
		buildPde(pde, platform.PageSize)
		buildPgd(pgd, uint64(pde_addr), platform.PageSize)
		buildPml4(pml4, uint64(pgd_addr), platform.PageSize)
	...
	BootCs := platform.SegmentValue{
		Base:     0,
		Limit:    0xffffffff,
		Selector: uint16(BootCsSelector), // @ 0x10
	...
	BootDs := platform.SegmentValue{
		Base:     0,
		Limit:    0xffffffff,
		Selector: uint16(BootDsSelector), // @ 0x18
		Dpl:      0,                      // Privilege level (kernel).
		Db:       1,                      // 32-bit segment?
		G:        1,                      // Granularity (page).
	...
	BootTr := platform.SegmentValue{
		Base:     0,
		Limit:    0xffffffff,
		Selector: uint16(BootTrSelector), // @ 0x20
		Dpl:      0,                        // Privilege level (kernel).
		Db:       1,                        // 32-bit segment?
		G:        1,                        // Granularity (page).
