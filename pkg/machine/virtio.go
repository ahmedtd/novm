...
const (
	VirtioDescFNext     = 1
	VirtioDescFWrite    = 2
	VirtioDescFIndirect = 4

	VRingUsedFNoNotify     = 1
	VRingAvailFNoInterrupt = 1
)
...
func vringSize(num uint32, align uint32) uint32 {
	size := num*16 + 4 + 2*num + 2
	size = (size + align - 1) & ^(align - 1)
	size += 4 + 8*num + 2
	return size
}

func (vr *vring) init(num uint32, mem []byte, align uint32) {
	vr.num = num
	vr.mem = mem
	vr.descOff = 0
	vr.availOff = int(num * 16)
	availEnd := vr.availOff + 4 + int(num*2) + 2
	vr.usedOff = (availEnd + int(align) - 1) & ^(int(align) - 1)
}

func (vr *vring) getIndex(index uint16) (addr uint64, length uint32, flags uint16, next uint16) {
	off := vr.descOff + int(index)*16
	addr = binary.LittleEndian.Uint64(vr.mem[off : off+8])
	length = binary.LittleEndian.Uint32(vr.mem[off+8 : off+12])
	flags = binary.LittleEndian.Uint16(vr.mem[off+12 : off+14])
	next = binary.LittleEndian.Uint16(vr.mem[off+14 : off+16])
	return
}

func (vr *vring) getBuf(consumed uint16) (hasBuf bool, flags uint16, index uint16, usedEvent uint16) {
	if len(vr.mem) == 0 || vr.num == 0 {
		return false, 0, 0, 0
	}
	availIdx := binary.LittleEndian.Uint16(vr.mem[vr.availOff+2 : vr.availOff+4])
	if consumed != availIdx {
		availEventOff := vr.usedOff + 4 + int(vr.num)*8
		if consumed+1 < availIdx {
			binary.LittleEndian.PutUint16(vr.mem[vr.usedOff:vr.usedOff+2], VRingUsedFNoNotify)
		} else {
			binary.LittleEndian.PutUint16(vr.mem[vr.usedOff:vr.usedOff+2], 0)
			binary.LittleEndian.PutUint16(vr.mem[availEventOff:availEventOff+2], consumed+1)
		}
		flags = binary.LittleEndian.Uint16(vr.mem[vr.availOff : vr.availOff+2])
		ringIdx := int(consumed % uint16(vr.num))
		index = binary.LittleEndian.Uint16(vr.mem[vr.availOff+4+ringIdx*2 : vr.availOff+6+ringIdx*2])
		usedEventOff := vr.availOff + 4 + int(vr.num)*2
		usedEvent = binary.LittleEndian.Uint16(vr.mem[usedEventOff : usedEventOff+2])
		return true, flags, index, usedEvent
	}
	return false, 0, 0, 0
}

func (vr *vring) putBuf(index uint16, length uint32) (evtInterrupt bool, noInterrupt bool) {
	if len(vr.mem) == 0 || vr.num == 0 {
		return false, false
	}
	usedIdx := binary.LittleEndian.Uint16(vr.mem[vr.usedOff+2 : vr.usedOff+4])
	ringIdx := int(usedIdx % uint16(vr.num))
	elemOff := vr.usedOff + 4 + ringIdx*8
	binary.LittleEndian.PutUint32(vr.mem[elemOff:elemOff+4], uint32(index))
	binary.LittleEndian.PutUint32(vr.mem[elemOff+4:elemOff+8], length)

	usedEventOff := vr.availOff + 4 + int(vr.num)*2
	usedEvent := binary.LittleEndian.Uint16(vr.mem[usedEventOff : usedEventOff+2])
	evtInterrupt = (usedEvent == usedIdx)

	availFlags := binary.LittleEndian.Uint16(vr.mem[vr.availOff : vr.availOff+2])
	noInterrupt = (availFlags & VRingAvailFNoInterrupt) != 0

	// Increment idx
	binary.LittleEndian.PutUint16(vr.mem[vr.usedOff+2:vr.usedOff+4], usedIdx+1)
	return evtInterrupt, noInterrupt
}
...
func (vchannel *VirtioChannel) processOne(n uint16) error {
	...
	for {
		// Read the entry.
		addr, length, buf_flags, next := vchannel.vring.getIndex(index)

		// Append our buffer.
		has_next := (buf_flags & VirtioDescFNext) != 0
		is_write := (buf_flags & VirtioDescFWrite) != 0
		is_indirect := (buf_flags & VirtioDescFIndirect) != 0

		// Do we have a buffer?
		if buf == nil {
			buf = NewVirtioBuffer(index, !is_write)
		}
		...
		if !has_next {
			...
			vchannel.Outstanding[buf.index] = true
...
func (vchannel *VirtioChannel) consumeOne() (bool, error) {
	...
	hasBuf, _, index, _ := vchannel.vring.getBuf(uint16(vchannel.Consumed))
	if hasBuf {
...
func (vchannel *VirtioChannel) ProcessOutgoing() error {
	...
	for buf := range vchannel.outgoing {
		...
		evt_interrupt, no_interrupt := vchannel.vring.putBuf(
			buf.index,
			uint32(buf.length))
		...
		delete(vchannel.Outstanding, buf.index)
...
func (vchannel *VirtioChannel) remap() error {
	...
	if vchannel.QueueAddress.Value != 0 {
		...
		vchannel_size := vringSize(
			uint32(vchannel.QueueSize.Value),
			uint32(platform.PageSize))
