...
func (vm *Vm) NewVcpu(id uint) (*Vcpu, error) {
	...
	kvm_run := (*kvmRun)(unsafe.Pointer(&mmap[0]))
...
func (vcpu *Vcpu) SetStepping(step bool) error {
	...
	var guest_debug kvmGuestDebug

	if step == vcpu.is_stepping {
		// Already set.
		return nil
	} else if step {
		guest_debug.control = uint32(IoctlGuestDebugEnable)
	} else {
